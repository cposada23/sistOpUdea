...
int main(int argc, char *argv[]) {
    ...
    if (argc != 3) {
        printf("Error: Numero de parametros invalido.\nDebe iniciar asi: ./taller.out <archivoEntrada.csv> <archivoSalida.csv>\n");
		exit(0);
    }
...
float getTotal(employee *emp) {
    ...
    for (i = 0; i < numEmployees; i++) {
        total += emp->salary + EXTRA*emp->horasExtra;
        emp++;
    }
...
void showEmployees(employee *emp, FILE *f) {
    ...
    fprintf(f, "|%-12s|%-12s|%-12s|\n", "Nombre", "Salario","horas Extra");
    for (i = 0; i < numEmployees; i++) {
        fprintf(f, "|%-12s|%-12.2f|%-12d|\n", emp->name, emp->salary, emp->horasExtra);
        emp++;
    }
}